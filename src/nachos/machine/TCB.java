...
public final class TCB
{
  ...
  {
    /*
     * We will not use synchronization here, because we're assuming that either
     * this is the first call to start(), or we're being called in the context
     * of another TCB. Since we only allow one TCB to run at a time, no
     * synchronization is necessary. The only way this assumption could be
     * broken is if one of our non-Nachos threads used the TCB code.
     */

    /*
     * Make sure this TCB has not already been started. If done is false, then
     * destroy() has not yet set javaThread back to null, so we can use
     * javaThread as a reliable indicator of whether or not start() has already
     * been invoked.
     */
    ...
    /*
     * Probably unnecessary sanity check: if this is not the first TCB, we make
     * sure that the current thread is bound to the current TCB. This check can
     * only fail if non-Nachos threads invoke start().
     */
    if (!isFirstTCB)
     ...
     * Whether or not this is the first TCB, it gets added to runningThreads,
     * and we save the target closure.
     */
    ...
    {
      /*
       * If this is not the first TCB, we have to make a new Java thread to run
       * it. Creating Java threads is a privileged operation.
       */
       ...
       * yield(). We do this by temporarily turning off the current TCB,
       * starting the new Java thread, and waiting for it to wake us up from
       * threadroot(). Once the new TCB wakes us up, it's safe to context switch
       * to the new TCB.
       */
    ...
    {
      /*
       * This is the first TCB, so we don't need to make a new Java thread to
       * run it; we just steal the current Java thread.
       */
  ...
  {
    /*
     * Probably unnecessary sanity check: we make sure that the current thread
     * is bound to the current TCB. This check can only fail if non-Nachos
    ...
    /*
     * There are some synchronization concerns here. As soon as we wake up the
     * next thread, we cannot assume anything about static variables, or about
     * any TCB's state. Therefore, before waking up the next thread, we must
     * latch the value of currentTCB, and set its running flag to false (so
     * that, in case we get interrupted before we call yield(), the interrupt
     * will set the running flag and yield() won't block).
     */
  ...
  private void threadroot ()
  {
    ...
    {
      /*
       * start() is waiting for us to wake it up, signalling that it's OK to
       * context switch to us. We leave the running flag false so that we'll
       * still run if a context switch happens before we go to sleep. All we
       * have to do is wake up the current TCB and then wait to get woken up by
       * contextSwitch() or destroy().
       */
    ...
    {
      /*
       * start() called us directly, so we just need to initialize a couple
       * things.
       */
   ...
   * destroyed instead, we check the <tt>done</tt> flag after waking up. If it
   * is set, the TCB that woke us up is waiting for an acknowledgement in
   * destroy(). Otherwise, we just set the current TCB to this TCB and return.
   */
  private void yield ()
  {
    waitForInterrupt();
   ...
   * from this TCB to another. We don't rely on <tt>currentTCB</tt>, since it is
   * updated by <tt>contextSwitch()</tt> before we get called.
   */
   ...
   * signalling the monitor bound to it. Used in the ping-pong process of
   * starting and destroying TCBs, as well as in context switching to this TCB.
   */
   ...
   * only in <tt>start(Runnable)</tt>, which can only be invoked once on each
   * TCB object. TCB objects are removed only in each of the <tt>catch</tt>
   * clauses of <tt>threadroot()</tt>, one of which is always invoked on thread
   * termination. The maximum number of threads in <tt>runningThreads</tt> is
   * limited to <tt>maxThreads</tt> by <tt>start(Runnable)</tt>. If
   * <tt>threadroot()</tt> drops the number of TCB objects in
   * <tt>runningThreads</tt> to zero, Nachos exits, so once the first TCB is
   * created, this vector is basically never empty.
   */
